...
- **学习新的题目**：从还没学过的卡片里挑一批（数量 = 「目标每轮学习数」，默认 5 张），连续答对设定的次数（默认 3 次，对应设置里的「连续正确次数」）后转入复习；这一轮学满设定的张数就结束，学到一半退出可以接着续学。
- **复习**：进入后把「当天到期的 + 上一轮答错还没补完连对的」卡片过一遍，答对一次就进入下一级；答错则掌握阶梯回到第 1 级重新开始，并且这张卡本轮还要再连续答对 N 次（N = 设置里的「连续正确次数」，不可单独设置）；只要还在同一天，退出再进来它仍留在本轮里。
