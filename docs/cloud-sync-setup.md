...
| `quiz_app_sync_meta` | 本地同步元数据（每个**题库**上次同步到哪 + 每个冲突**第一次看到**的时刻），传上去没有意义 |
...
- **冲突**（同一个**题库**两边都改过）：**绝不自动选边**，而且**在你做出选择之前，
  ...
  两个按钮下面各有一行时间小字（与按钮左边缘对齐，悬停看绝对时间）：**冲突发生时间**
  （这台设备第一次看到这个冲突的时刻，写一次就不再动——冲突之后你往往还在继续刷题，
  本地修改时间会一直往前跑，显示的永远是「刚刚」，那个时间没有任何信息量）和
  **上传时间**（云端那份快照最后被改动的时间，也就是 Gitee 那条 Gist 的 `updated_at`，
  粒度是整条 Gist）。**改动更晚的那一边是实心按钮**，另一边是描边的：
  看一眼时间就知道该按哪个。
  「冲突发生时间」**记在 `quiz_app_sync_meta` 里**（`conflicts` 段），所以刷新页面、
  关掉再打开云同步都还是原来那个时间；那次冲突裁决并同步之后，这条记录跟着作废。
- **怎么判断两边谁改了**：
