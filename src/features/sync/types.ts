...
export interface SyncStatus {
  ...
  /** 最近一次操作的结果说明（成功 / 失败） */
  message: string;
  /** 最近一次操作完成的时间（毫秒）——自检、对账也算，只用于排查 */
  at: number;
  /**
   * 最近一次**真的同步**完成的时间（毫秒）；从没同步过就是 0。
   *
   * 和 `at` 分开是必须的：`testConnection` 也会写 `at`，于是「点一下测试连接，
   * 卡片上的『上次同步』就跳到刚刚」——看起来像它偷偷同步了一次。它没有：
   * 自检只读云端（外加回填 gistUrl），一个字节都不写。
   */
  lastSyncAt: number;
  /** 云端共几个文件 */
  remoteCount: number;
